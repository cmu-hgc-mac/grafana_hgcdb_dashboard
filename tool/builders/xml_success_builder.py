...
class XMLSuccessBuilder:
    def __init__(self, datasource_uid, timezone = 'America/New_York'):
        ...
        self.table_sql = f"""
        WITH module_info_latest AS (
            SELECT DISTINCT ON (module_name) module_name, xml_upload_success AS latest_xml_upload_success
            FROM module_info
            WHERE $__timeFilter(module_info.assembled)
            ORDER BY module_name, module_no DESC
        ),
        module_info_failed AS (
        ...
        proto_assembly_latest AS (
            SELECT DISTINCT ON (proto_assembly.proto_name) module_info.module_name, proto_assembly.xml_upload_success AS latest_xml_upload_success
            FROM proto_assembly
            JOIN module_info ON proto_assembly.proto_name = module_info.proto_name
            ORDER BY proto_assembly.proto_name, proto_assembly.proto_no DESC
        ),
        ...
        proto_inspect_latest AS (
            SELECT DISTINCT ON (proto_inspect.proto_name) module_info.module_name, proto_inspect.xml_upload_success AS latest_xml_upload_success
            FROM proto_inspect
            JOIN module_info ON proto_inspect.proto_name = module_info.proto_name
            ORDER BY proto_inspect.proto_name, proto_inspect.proto_no DESC
        ),
        ...
        module_assembly_latest AS (
            SELECT DISTINCT ON (module_assembly.module_name) module_assembly.module_name, module_assembly.xml_upload_success AS latest_xml_upload_success
            FROM module_assembly
            ORDER BY module_assembly.module_name, module_assembly.module_no DESC
        ),
        ...
        module_inspect_latest AS (
            SELECT DISTINCT ON (module_inspect.module_name) module_inspect.module_name, module_inspect.xml_upload_success AS latest_xml_upload_success
            FROM module_inspect
            ORDER BY module_inspect.module_name, module_inspect.module_no DESC
        ),
        ...
        module_iv_latest AS (
            SELECT DISTINCT ON (module_iv_test.module_name) module_iv_test.module_name, module_iv_test.xml_upload_success AS latest_xml_upload_success
            FROM module_iv_test
            WHERE status = 7 OR status = 8
            ORDER BY module_iv_test.module_name, module_iv_test.module_no DESC
        ),
        module_iv_failed AS (
        ...
        module_pedestal_latest AS (
            SELECT DISTINCT ON (module_pedestal_test.module_name) module_pedestal_test.module_name, module_pedestal_test.xml_upload_success AS latest_xml_upload_success
            FROM module_pedestal_test
            WHERE status = 7 OR status = 8
            ORDER BY module_pedestal_test.module_name, module_pedestal_test.module_no DESC
        ),
        ...
        module_grade_latest AS (
            SELECT DISTINCT ON (module_qc_summary.module_name) module_qc_summary.module_name, module_qc_summary.xml_upload_success AS latest_xml_upload_success
            FROM module_qc_summary
            ORDER BY module_qc_summary.module_name, module_qc_summary.mod_qc_no DESC
        ),
        ...
        hxb_inspect_latest AS (
            SELECT DISTINCT ON (hxb_inspect.hxb_name) module_info.module_name, hxb_inspect.xml_upload_success AS latest_xml_upload_success
            FROM hxb_inspect
            JOIN module_info ON hxb_inspect.hxb_name = module_info.hxb_name
            ORDER BY hxb_inspect.hxb_name, hxb_inspect.hxb_no DESC
        ),
        ...
        bp_inspect_latest AS (
            SELECT DISTINCT ON (bp_inspect.bp_name) module_info.module_name, bp_inspect.xml_upload_success AS latest_xml_upload_success
            FROM bp_inspect
            JOIN module_info ON bp_inspect.bp_name = module_info.bp_name
            ORDER BY bp_inspect.bp_name, bp_inspect.bp_no DESC
        ),
        ...
        hxb_pedestal_latest AS (
            SELECT DISTINCT ON (hxb_pedestal_test.hxb_name) module_info.module_name, hxb_pedestal_test.xml_upload_success AS latest_xml_upload_success
            FROM hxb_pedestal_test
            JOIN module_info ON hxb_pedestal_test.hxb_name = module_info.hxb_name
            ORDER BY hxb_pedestal_test.hxb_name, hxb_pedestal_test.hxb_no DESC
        ),
        ...
        sen_inspect_latest AS (
            SELECT DISTINCT ON (sensor.sen_name) module_info.module_name, sensor.xml_upload_success AS latest_xml_upload_success
            FROM sensor
            JOIN module_info ON sensor.sen_name = module_info.sen_name
            ORDER BY sensor.sen_name, sensor.sen_no DESC
        ),
        ...
        back_wirebond_latest AS (
            SELECT DISTINCT ON (back_wirebond.module_name) back_wirebond.module_name, back_wirebond.xml_upload_success AS latest_xml_upload_success
            FROM back_wirebond
            ORDER BY back_wirebond.module_name, back_wirebond.module_no DESC
        ),
        ...
        back_encap_latest AS (
            SELECT DISTINCT ON (back_encap.module_name) back_encap.module_name, back_encap.xml_upload_success AS latest_xml_upload_success
            FROM back_encap
            ORDER BY back_encap.module_name, back_encap.module_no DESC
        ),
        ...
        front_wirebond_latest AS (
            SELECT DISTINCT ON (front_wirebond.module_name) front_wirebond.module_name, front_wirebond.xml_upload_success AS latest_xml_upload_success
            FROM front_wirebond
            ORDER BY front_wirebond.module_name, front_wirebond.module_no DESC
        ),
        ...
        front_encap_latest AS (
            SELECT DISTINCT ON (front_encap.module_name) front_encap.module_name, front_encap.xml_upload_success AS latest_xml_upload_success
            FROM front_encap
            ORDER BY front_encap.module_name, front_encap.module_no DESC
        ),
        ...
        bond_pull_latest AS (
            SELECT DISTINCT ON (bond_pull_test.module_name) bond_pull_test.module_name, bond_pull_test.xml_upload_success AS latest_xml_upload_success
            FROM bond_pull_test
            ORDER BY bond_pull_test.module_name, bond_pull_test.module_no DESC
        ),
        bond_pull_failed AS (
        ...
        LEFT JOIN module_info_latest
            ON module_info_failed.module_name = module_info_latest.module_name
        LEFT JOIN proto_assembly_failed
            ON module_info_failed.module_name = proto_assembly_failed.module_name
        LEFT JOIN proto_assembly_latest
            ON module_info_failed.module_name = proto_assembly_latest.module_name
        LEFT JOIN proto_inspect_failed
            ON module_info_failed.module_name = proto_inspect_failed.module_name
        LEFT JOIN proto_inspect_latest
            ON module_info_failed.module_name = proto_inspect_latest.module_name
        LEFT JOIN module_assembly_failed
            ON module_info_failed.module_name = module_assembly_failed.module_name
        LEFT JOIN module_assembly_latest
            ON module_info_failed.module_name = module_assembly_latest.module_name
        LEFT JOIN module_inspect_failed
            ON module_info_failed.module_name = module_inspect_failed.module_name
        LEFT JOIN module_inspect_latest
            ON module_info_failed.module_name = module_inspect_latest.module_name
        LEFT JOIN module_iv_failed
            ON module_info_failed.module_name = module_iv_failed.module_name
        LEFT JOIN module_iv_latest
            ON module_info_failed.module_name = module_iv_latest.module_name
        LEFT JOIN module_pedestal_failed
            ON module_info_failed.module_name = module_pedestal_failed.module_name
        LEFT JOIN module_pedestal_latest
            ON module_info_failed.module_name = module_pedestal_latest.module_name
        LEFT JOIN module_grade_failed
            ON module_info_failed.module_name = module_grade_failed.module_name
        LEFT JOIN module_grade_latest
            ON module_info_failed.module_name = module_grade_latest.module_name
        LEFT JOIN hxb_inspect_failed
            ON module_info_failed.module_name = hxb_inspect_failed.module_name
        LEFT JOIN hxb_inspect_latest
            ON module_info_failed.module_name = hxb_inspect_latest.module_name
        LEFT JOIN bp_inspect_failed
            ON module_info_failed.module_name = bp_inspect_failed.module_name
        LEFT JOIN bp_inspect_latest
            ON module_info_failed.module_name = bp_inspect_latest.module_name
        LEFT JOIN hxb_pedestal_failed
            ON module_info_failed.module_name = hxb_pedestal_failed.module_name
        LEFT JOIN hxb_pedestal_latest
            ON module_info_failed.module_name = hxb_pedestal_latest.module_name
        LEFT JOIN sen_inspect_failed
            ON module_info_failed.module_name = sen_inspect_failed.module_name
        LEFT JOIN sen_inspect_latest
            ON module_info_failed.module_name = sen_inspect_latest.module_name
        LEFT JOIN back_wirebond_failed
            ON module_info_failed.module_name = back_wirebond_failed.module_name
        LEFT JOIN back_wirebond_latest
            ON module_info_failed.module_name = back_wirebond_latest.module_name
        LEFT JOIN back_encap_failed
            ON module_info_failed.module_name = back_encap_failed.module_name
        LEFT JOIN back_encap_latest
            ON module_info_failed.module_name = back_encap_latest.module_name
        LEFT JOIN front_wirebond_failed
            ON module_info_failed.module_name = front_wirebond_failed.module_name
        LEFT JOIN front_wirebond_latest
            ON module_info_failed.module_name = front_wirebond_latest.module_name
        LEFT JOIN front_encap_failed
            ON module_info_failed.module_name = front_encap_failed.module_name
        LEFT JOIN front_encap_latest
            ON module_info_failed.module_name = front_encap_latest.module_name
        LEFT JOIN bond_pull_failed
            ON module_info_failed.module_name = bond_pull_failed.module_name
        LEFT JOIN bond_pull_latest
            ON module_info_failed.module_name = bond_pull_latest.module_name
        ...
        "templating": {
            "list": [
                ...
                },
                {
                    ...
                    "label": "Check Latest Only",
                    "name": "check_latest_only",
                    "options": [
                        {
                            "selected": True,
                            "text": "Yes",
