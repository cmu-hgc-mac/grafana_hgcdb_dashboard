...
from tool import PanelBuilder, FilterBuilder, InputBuilder, DashboardBuilder, ComponentsLookUpFormBuilder, HexmapPlotsBuilder, OffsetPlotsBuilder, GeneralInfoBuilder, ModuleAssemblyBuilder, XMLSuccessBuilder, ModuleGradesBuilder, MMTSBuilder
...
mmts_builder = MMTSBuilder(GF_MMTS_DS_UID, TIME_ZONE)
...
for config in filelist:
    ...
    for dashboard in dashboards:
        ...
        elif dashboard_title == "MMTS Info":
            dashboard_json = mmts_builder.generate_dashboard_json()
            # Export the dashboard json to a file
            file_name = config.split(".")[0]
            dashboard_builder.save_dashboard_json(dashboard, dashboard_json, file_name)
            continue
